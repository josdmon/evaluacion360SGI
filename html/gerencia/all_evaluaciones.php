            ...
            <?php 
              if(false != $_historialevaluacion) {
                foreach($_historialevaluacion as $id_evaluador => $array_evaluador) {
                  echo '<a class="btn btn-primary btn-lg btn-block" role="button" aria-pressed="true" href="?view=gerente&mode=resultado_general&id='.$id_evaluador.'">Esta es la Evaluación N°: '.$_historialevaluacion[$id_evaluador]['id'].' Con fecha '.$_historialevaluacion[$id_evaluador]['fecha'].'</a>';
                }
              } else {
